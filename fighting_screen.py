...
class FightingScreen(Screen):
    ...
    def spawn_enemies(self):
        num_enemies = [Enemy(random.randint(1, 30), random.randint(1, 30), random.randint(1, 30), random.randint(1, 30), random.randint(1, 30), )]random.randint(0,11)
...
class Enemy:
    def __init__(self, posX:int, posY:int, sizeX:int, sizeY:int, speed:int, imgpath:str):
        ...
        img_scaled = pygame.transform.scale(pygame.image.load(os.path.join(self.__imgpath).replace(path_separator[system()][0]).convert_alpha(), self.__sizeX, self.__sizeY))
